package io.github.atreia108.vega.math;

/**
 * Quaternion model.
 *
 * These are a start on some very simple quaternion routines. Quaternions are
 * often used as a non-singular method for representing rotations or attitudes.
 * There are a lot more methods required to use quaternions in the propagation
 * of the rotational state of an object. However, these will provide the basic
 * methods for going to and from a more traditional transformation matrix
 * representation.
 *
...
public class Quaternion
{
	...
	void left_quat_from_eigen_rotation(double eigen_angle, // In: r Eigen angle
	...
	void scale(final double fact) // In: -- Scale factor
	{
		scalar *= fact;
	...
	final void scale(final double fact, // In: -- Scale factor
	...
	final void normalize(Quaternion quat) // Out: -- Normalized quaternion
	{
		quat = this;
		quat.normalize();
	}
	...
	// Purpose: (Form the normalized quaternion, leaving original intact.)
	final void normalize_integ(Quaternion quat) // Out: -- Normalized quaternion
	{
		quat = this;
	...
	final void conjugate(Quaternion quat) // Out: -- Conjugated quaternion
	{
		quat.scalar = scalar;
	...
	// prod = this * quat.)
	final void multiply(final Quaternion quat, // In: -- Right multiplicand
	...
	// Purpose: (Post-multiply this quaternion by another quaternion:
	// this = this * quat.)
	void multiply(final Quaternion quat) // In: -- Right multiplicand
	{
		double v_dot_qv;
	...
	// prod = conj(this) * quat.)
	final void conjugate_multiply(final Quaternion quat, // In: -- Right multiplicand
			Quaternion prod) // Out: -- Quaternion product
	{
		prod.scalar = scalar * quat.scalar + Vector3.dot(vector, quat.vector);
		Vector3.scale(quat.vector, scalar, prod.vector);
	...
	// prod = this * conj(quat).)
	final void multiply_conjugate(final Quaternion quat, // In: -- Right multiplicand
			Quaternion prod) // Out: -- Quaternion product
	{
		prod.scalar = scalar * quat.scalar + Vector3.dot(vector, quat.vector);
	...
	// Purpose: (Post-multiply this quaternion by another's conjugate:
	// this = this * conj(quat).)
	void multiply_conjugate(final Quaternion quat) // In: -- Right multiplicand
	{
		double v_dot_qv;
		double[] v_cross_qv = new double[3];
	...
	// Purpose: (Post-multiply this quaternion's conjugate by another quaternion:
	// this = conj(this) * quat.)
	void conjugate_multiply(final Quaternion quat) // In: -- Right multiplicand
	{
		double v_dot_qv;
		double[] v_cross_qv = new double[3];
	...
	// this = quat * this.)
	void multiply_left(final Quaternion quat) // In: -- Right multiplicand
	{
		double qv_dot_v;
	...
	// this = conj(quat) * this.)
	void multiply_left_conjugate(final Quaternion quat) // In: -- Right multiplicand
	{
		double qv_dot_v;
		double[] qv_cross_v = new double[3];
	...
	// prod = [0, vec] * quat.)
	final void multiply_vector_left(final double[] vec, // In: -- Right multiplicand
			Quaternion prod) // Out: -- Quaternion product
	{
	...
	// quaternion, the latter represented by a vector:
	// prod = quat * [0, vec].)
	final void multiply_vector_right(final double[] vec, // In: -- Right multiplicand
			Quaternion prod) // Out: -- Quaternion product
	{
		prod.scalar = -Vector3.dot(vector, vec);
		Vector3.scale(vec, scalar, prod.vector);
	...
	final void eigen_compare(final Quaternion quat, // In: -- Quaternion to compare to
	...
	final void compute_left_quat_deriv(final double[] ang_vel, // In: r/s Angular velocity
	...
	// Purpose: (Compute the time derivative of a left quaternion.)
	void compute_left_quat_deriv(final double[] quat, // In: -- Quaternion as 4-vector
	...
	public void left_quat_from_transform(final double[][] T)
	{
		...
		/*
		 * Find the largest of the trace (meth = -1) and the three diagonal elements of
		 * 'a' (meth = 0, 1, or 2).
		 */
		meth = -1;
		tmax = tr;
	...
	{
		System.out.println(scalar + " | " + vector[0] + ", " + vector[1] + ", " + vector[2]);
