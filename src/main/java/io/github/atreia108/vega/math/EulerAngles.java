 ...
 * Euler Angle model.
 *
 * These are a start on some very simple Euler angle routines. Euler angles are
 * often used as a potentially singular method for representing rotations or
 * attitudes. There are a lot more methods required to use Euler angles in the
 * propagation of the rotational state of an object. However, these will provide
 * the basic methods for going to and from a more traditional transformation
...
public class EulerAngles
{
	...
	public enum EulerSequence
	{
		...
		EulerXYZ(new int[] { 0, 1, 2 }, 0, 2, true, true), EulerXZY(new int[] { 0, 2, 1 }, 0, 1, false, true), EulerYZX(new int[] { 1, 2, 0 }, 1, 0, true, true), EulerYXZ(new int[] { 1, 0, 2 }, 1, 2, false, true), EulerZXY(new int[] { 2, 0, 1 }, 2, 1, true, true), EulerZYX(new int[] { 2, 1, 0 }, 2, 0, false, true),

		// Astronomical Euler sequences, again named by axes
		EulerXYX(new int[] { 0, 1, 0 }, 2, 2, true, false), EulerXZX(new int[] { 0, 2, 0 }, 1, 1, false, false), EulerYZY(new int[] { 1, 2, 1 }, 0, 0, true, false), EulerYXY(new int[] { 1, 0, 1 }, 2, 2, false, false), EulerZXZ(new int[] { 2, 0, 2 }, 1, 1, true, false), EulerZYZ(new int[] { 2, 1, 2 }, 0, 0, false, false);

		private final int[] indices = new int[3]; /*
													 * -- The axes about which the rotations are performed in the order in which the
													 * rotations are performed, with X=0, Y=1, Z=2. For example, an XYZ or roll
													 * pitch yaw sequence is {0,1,2} while a ZXZ sequence is {2,0,2}.
													 */

		private final int alternate_x; /*
										 * -- The initial element of the sequence for aerodynamics sequences, but the
										 * index of the omitted axis for astronomical sequences. For example, the
										 * omitted axis in a ZXZ sequence is Y=1.
										 */

		private final int alternate_z; /*
										 * -- The final element of the sequence for aerodynamics sequences, but the
										 * index of the omitted axis for astronomical sequences.
										 */

		private final boolean is_even_permutation; /*
													 * -- Indicates whether the 3-axis rotation sequence generated by replacing the
													 * final element of the sequence with the one axis not specified by the first
													 * two elements of the sequence is an even (true) permutation or odd permutation
													 * (false) of XYZ. The alternative 3-axis sequence is identical to the original
													 * sequence in the case of aerodynamics sequences. The astronomical ZXZ sequence
													 * becomes ZXY via this replacement rule. Since ZXY is an even permutation of
													 * XYZ, the is_even_permutation member for a ZXZ sequence is true.
													 */

		private final boolean is_aerodynamics_sequence; /*
														 * -- True if the sequence is an aerodynamics sequence such as XYZ; false for an
														 * astronomical sequence such as ZXZ.
														 */
	...
	public EulerAngles(final EulerSequence new_sequence, final double[] new_angles)
	{
	...
	public EulerAngles(final EulerSequence new_sequence, final double[][] trans_matrix)
	{
		this.set_sequence(new_sequence);
	...
	public EulerAngles(final EulerSequence new_sequence, final Quaternion quaternion)
	{
	...
	public void set_sequence(EulerSequence new_sequence)
	{
		if (new_sequence != this.sequence)
		{
	...
	public void set_angles(double[] new_angles)
	{
	...
	public void set_from_left_quaternion(final Quaternion quaternion)
	{
		double[][] trans_matrix = new double[3][3];
		quaternion.left_quat_to_transform(trans_matrix);
		this.set_from_matrix(trans_matrix);
	...
	/*
	 * Purpose: (Extract an Euler sequence from the transformation matrix. A
	 * transformation matrix constructed from an XYZ Euler sequence is of the
	 * form @f[ \left[\array{ccc} \cos\psi\cos\theta & \cdots & \cdots \\
	 * -\sin\psi\cos\theta & \cdots & \cdots \\ \sin\theta & -\cos\theta\sin\phi &
	 * \cos\theta\cos\phi \endarray\right]
	 * 
	 * @f] Note that the [2][0] element of the matrix depends on theta only. The
	 * other two elements of the leftmost column are simple terms that depend on
	 * theta and psi only, and the other two elements of the bottommost row are
	 * simple terms that depend on theta and phi only. Those five elements are the
	 * key to extracting an XYZ Euler sequence from a transformation matrix. The
	 * same principle applies to all twelve of the Euler sequences: Five key
	 * elements contain all of the information needed to extract the desired
	 * sequence. The location and form of those key elements of course depends on
	 * the sequence.
	 * 
	 ...
	 * XYZ Euler sequence, when theta=pi/2 the matrix becomes @f[ \left[\array{ccc}
	 * 0 & \sin(\phi+\psi) & -cos(\phi+\psi) \\ 0 & \cos(\phi+\psi) &
	 * \sin(\phi+\psi) \\ 1 & 0 & 0 \endarray\right]
	 * 
	 * @f] In this case there no way to determine both phi and psi; all that can be
	 ...
	 * sequence to be extracted from the matrix even in the case of gimbal lock. The
	 * same principle once again applies to all twelve sequences.
	 * 
	 * In summary, for a transformation matrix corresponding to an XYZ sequence, -
	 * The [2][0] element of the matrix specifies theta. - The [1][0] and [0][0]
	 * elements of the matrix specify psi. - The [2][1] and [2][2] elements of the
	 * matrix specify phi. These psi and phi values are valid only when gimbal lock
	 * is not present. - The [1][2] and [1][1] elements of the matrix specify phi in
	 * the case of gimbal lock.
	 * 
	 ...
	 * transformation matrix. This information is captured in the EulerInfo array
	 * Euler_info defined at the head of this file. With a reference <tt>info</tt>
	 * to the appropriate element of this array, - The
	 * [info.indices[2]][info.indices[0]] element of the matrix specifies the angle
	 * theta. - The [info.indices[1]][info.indices[0]] and
	 * [info.alternate_x][info.indices[0]] elements of the matrix specify the angle
	 * psi when gimbal lock is not present. - The [info.indices[2]][info.indices[1]]
	 * and [info.indices[2]][info.alternate_z] elements of the matrix specify the
	 * angle phi when gimbal lock is not present. - The
	 * [info.indices[1]][info.alternate_z] and [info.indices[1]][info.indices[1]]
	 * elements of the matrix specify angle phi when gimbal lock is present.)
	 * 
	 * Assumptions and limitations: ((To within numerical accuracy, the
	 * transformation matrix in the Orientation object @e is a proper transformation
	 * matrix: - The magnitude of each row and column vector is nearly one. - The
	 * inner product of any two different rows / two different columns of the matrix
	 * nearly zero. - The determinant of the matrix is nearly one. - An element
	 * whose value is outside the range [-1,1] is only slightly outside that range
	 * and the deviation is numerical.))
	 */
	public void set_from_matrix(final double[][] trans) // In: -- Transformation matrix
	{
	...
	/*
	 * Purpose: (Compute the transformation matrix from the Euler sequence. The
	 * matrix is formed by generating a sequence of three simple transformation
